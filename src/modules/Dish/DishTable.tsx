...
import { DishSidebar } from "./DishSidebar";
...
export const DishTable = ( props: any ) => {
  const { items, loading,  } = props;

  const { deleteForm, clearDishDataHook, DishLoading, DishSubmit,} = useDishHook();
  const [ showModal, setShowModal ] = useState<boolean>( false );
  const [ showDishItemViewSidebar, setShowDishItemViewSidebar ] = useState<boolean>(
    false
  );

  const [ dishId, setDishId ] = useState<any>();
    ...
    return (
      <>
        ...
        text
          ...
          onClick={ () => {
            setDishId( row?.dish_id );
            setShowDishItemViewSidebar( true );
          } }
          icon="pi pi-pencil"
        ></Button>
  ...
  return (
    <>
      ...
      <Sidebar
            ...
            position="right"
            visible={ showDishItemViewSidebar }
            onHide={ () => setShowDishItemViewSidebar( false ) }
            showCloseIcon={ !DishSubmit }
          >
            <DishSidebar
              dishId={ dishId }
              setDishId={ setDishId }
              setShowSidebar={ setShowDishItemViewSidebar }
            />
          </Sidebar>
      <div>
        <div>
